...
Sprite coelho;			// Sprite do coelho no jogo
...
int main()
{
	...
	Sprite espada_1; // Sprite para a espada 1
	...
	espada_1.setFallSpeed(0.2f);
	espada_2.setFallSpeed(0.2f);
	cenoura.setFallSpeed(0.1f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{
			float coelhoSpeed = 200.0f; // Velocidade de movimento do coelho

			// Verifica se a tecla D ou a seta para a direita foi pressionada
			if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS)
			{
				coelho.switchSide(texID_coelhoR);		   // Muda a textura do coelho para a direção direita
				coelho.moveRight(coelhoSpeed * deltaTime); // Move o coelho para a direita
			}
			// Verifica se a tecla A ou a seta para a esquerda foi pressionada
			if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS || glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
			{
				coelho.switchSide(texID_coelhoL);		  // Muda a textura do coelho para a direção esquerda
				coelho.moveLeft(coelhoSpeed * deltaTime); // Move o coelho para a esquerda
			}
		}
		...
		{
			background.desenhar(); // Desenha o fundo
			espada_1.desenhar();   // Desenha a espada 1
			espada_2.desenhar();   // Desenha a espada 2
			cenoura.desenhar();	   // Desenha a cenoura
			placar.desenhar();	   // Desenha o placar
			coelho.desenhar();	   // Desenha o sprite do coelho
			...
			// Verifica colisão entre o coelho e os objetos
			...
			if (aux_cenoura) // Se colidir com a cenoura
			{
				...
				if (pontos >= quantPont) // Se atingir a pontuação máxima
				{
					...
					cenoura.cair(true); // Reinicia a posição da cenoura
			}
			if (auxespada_1 || auxespada_2) // Se colidir com as espadas
			{
...
GLuint loadTexture(const string &texturePath)
{
	...
	{
		cout << "Falha ao carregar a textura: " << texturePath << endl;
	}
